...
export default function VetProfileScreen({ navigation }) {
  ...
  const fetchProfile = async () => {
    ...
    try {
      ...
      else setLoadError({ type: ERROR_TYPES.UNKNOWN });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    if (!form.clinic_name.trim()) {
      Alert.alert('Uyarı', 'Klinik adı boş bırakılamaz.');
      return;
    }
    if (!form.clinic_address.trim()) {
      Alert.alert('Uyarı', 'Adres boş bırakılamaz.');
      return;
    }
      ...
      >
        ...
        <View style={styles.avatarContainer}>
          ...
          <Text style={styles.nameLabel}>
            {profile?.clinic_name || 'Klinik Adı'}
          </Text>
          ...
          <View style={styles.field}>
            ...
            {form.latitude && form.longitude ? (
              <Text style={styles.locationSet}>
                ✅ Konum ayarlandı (
                {(form.latitude || profile.latitude)?.toFixed(4)},{' '}
                {(form.longitude || profile.longitude)?.toFixed(4)})
              </Text>
            ) : (
              <Text style={styles.locationNotSet}>
                ⚠️ Konum henüz ayarlanmadı
              </Text>
            )}
            {editMode && (
        ...
        <View style={{ gap: 12 }}>
          ...
          {/* Çıkış Yap - VetHome'dan buraya taşındı */}
...
const styles = StyleSheet.create({
  ...
  hintText: { fontSize: 11, color: '#aaa', marginTop: 4, textAlign: 'right' },
  locationSet: {
    fontSize: 13,
    color: '#28a745',
    fontWeight: '600',
  ...
  locationBtnMap: { backgroundColor: Colors.primary },
  ...
  changePasswordBtnText: { color: '#fff', fontWeight: '700', fontSize: 15 },
  logoutBtn: {
    borderRadius: 10,
    paddingVertical: 14,
    alignItems: 'center',
